...
city_data = [
      ...
      fetch(apiUrl)
          ...
          .then(data => {
              ...
              for (let i = 0; i < events.length; i++) {
                  ...
                  const eventLng = event._embedded.venues[0].location.longitude;
  
                  // Create a marker for the event
                  const eventMarker = L.marker([eventLat, eventLng]).addTo(myMap);
  ...
  for (let i = 0; i < city_data.length; i++) {
      ...
      marker.on('click', function () {
          ...
          //     if (layer !== marker) {
          //         eventMarkerGroupMarkerGroup.removeLayer(layer);
          //     }
          // });
  
          // Clear all markers and event markers from the map.
          eventMarkerGroup.eachLayer(function (layer) {
              if (layer !== myMap) {
